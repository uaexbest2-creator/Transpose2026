...
const App = () => {
  ...
  const [locationAlert, setLocationAlert] = useState<{message: string, zone: string} | null>(null);
  ...
  const simulateStopDetection = () => {
    ...
    // Simulate motion sensor stop trigger
    navigator.geolocation.getCurrentPosition(async (pos) => {
      const { latitude, longitude } = pos.coords;
      
      try {
        // Use Gemini 2.5 Flash with Maps tool to determine parking context
        const response = await ai.models.generateContent({
          model: 'gemini-2.5-flash',
          contents: "Based on my current coordinates in the UAE, am I in a paid parking area? Identify if it's Abu Dhabi or Dubai, the zone number (like C313), and the parking type (Premium/Standard/Paid). Format your answer like 'Premium parking detected in Abu Dhabi' or 'Parking detected in C313 in Dubai'.",
          config: {
            tools: [{ googleMaps: {} }],
            toolConfig: {
              retrievalConfig: {
        ...
        const info = response.text || "Parking detected nearby.";
        
        // Extract a zone for logic
        const zoneMatch = info.match(/([A-Z0-9]{3,5})/i);
        const zone = zoneMatch ? zoneMatch[1].toUpperCase() : "AUTO";
        ...
        setLocationAlert({ 
          message: "Parking zone detected near your current location. Would you like to check in?", 
          zone: "AUTO" 
        });
      } finally {
        setLoading(false);
  ...
  const handleStartParking = (zoneOverride?: string) => {
    const newSession: ParkingSession = {
      ...
      location: 'Detected Smart Zone',
      startTime: new Date().toISOString(),
      cost: 4,
      status: 'active',
      zone: zoneOverride || detectedZone || '332C',
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col pb-24">
      ...
      <main className="flex-1 p-4 max-w-2xl mx-auto w-full space-y-6">
        ...
        {/* SMART LOCATION ALERT (DETECTION NOTIFICATION) */}
          ...
          <div className="bg-amber-400 p-6 rounded-[2.5rem] shadow-2xl border-4 border-amber-300 animate-in slide-in-from-top-12 duration-500">
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1 space-y-2">
                <h4 className="font-black text-indigo-950 uppercase text-xs tracking-widest flex items-center gap-1.5"><AlertCircle size={14}/> Intelligent Check-in</h4>
                <p className="text-indigo-900 font-bold leading-tight text-lg">{locationAlert.message}</p>
                <div className="flex gap-2 pt-2">
                  <button 
                    onClick={() => handleStartParking(locationAlert.zone)}
                    className="bg-indigo-950 text-white px-8 py-3 rounded-xl font-black text-xs uppercase tracking-widest shadow-xl active:scale-95 transition-all"
                  >
                    Pay & Park
                  </button>
                  <button 
                    onClick={() => setLocationAlert(null)}
                    className="bg-white/30 text-indigo-950 px-4 py-3 rounded-xl font-bold text-xs"
                  >
                    Not Now
                  </button>
                </div>
          ...
          <div className="space-y-6">
            ...
            <section className="bg-white rounded-[2.5rem] p-8 shadow-sm border border-slate-200 flex flex-col items-center text-center gap-6 relative overflow-hidden">
               ...
               <div>
                 ...
                 <p className="text-slate-500 font-medium">
                    {isParked 
                      ...
                      : (detectedZone ? `Detected Zone ${detectedZone}` : 'Tracking your route in real-time.')}
                 </p>
               </div>
               ...
               <div className="w-full space-y-3">
                {/* Fixed type incompatibility by wrapping handlers in arrow functions to prevent passing the event object as an argument */}
                <button 
                ...
                >
                  {isParked ? 'End Session' : 'Start Parking Manually'}
                </button>
                {!isParked && (
                  <button 
            ...
            <section className="bg-amber-50 rounded-[2.5rem] p-6 border border-amber-200 shadow-sm">
              ...
              {loading ? (
                ...
                <p className="text-sm text-amber-900/80 leading-relaxed font-medium italic">
                  "{aiSuggestion || "Awaiting further fleet data for deeper insights."}"
                </p>
              )}
            </section>
